...
#	os.rename(my_file, base + '.csv')
	outfile=os.path.splitext(my_file)[0]+'.ics'
        if isdir(expanduser('~/Desktop')):
            f = open(easygui.filesavebox(msg='Save .ics File', title='', default=expanduser('/Users/cian/Google Drive/Cian_Noor/Rota/') + outfile, filetypes=['*.ics']), 'wb')
        else:
